...
new_data_train=an.AnnData(X=data.X[id_train,:][:,id_gex],obs=data.obs.iloc[id_train,:])
new_data_train.layers["counts"]=data.layers["counts"][id_train,:][:,id_gex]
new_data_test=an.AnnData(X=data.X[id_test,:][:,id_gex],obs=data.obs.iloc[id_test,:])
