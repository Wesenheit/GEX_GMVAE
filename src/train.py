...
def plot(test_data, name, dim, title):
    isol_lab_asw = scib.metrics.isolated_labels_asw(
        test_data, label_key="cell_type", batch_key="batch", embed="embed"
    )
    ...
    batch_sil = scib.metrics.silhouette_batch(
        test_data, batch_key="batch", label_key="cell_type", embed="embed"
    )
    ...
    for res in resolutions:
        ...
        new_nmi = scib.metrics.nmi(
            test_data, cluster_key="leiden", label_key="cell_type"
        )
        nmi = max(nmi, new_nmi)
    ari = 0
    ...
    for num in clust_num:
        ...
        ari_new = scib.metrics.ari(
            test_data, label_key="cell_type", cluster_key="kmean"
        )
        ari = max(ari, ari_new)
    ...
    data = pd.DataFrame(
        {
            "UMAP 1": values[:, 0],
            "UMAP 2": values[:, 1],
            "Cell type": test_data.obs["cell_type"],
        }
    )
    ...
    ax.text(
        0.5,
        -0.15,
        "Cell type ASW: {0:.3f}, Cell type NMI: {1:.3f}, Cell type ARI {2:.3f}".format(
            isol_lab_asw, nmi, ari
        ),
    ...
    ax.text(
        0.5,
        -0.2,
        "Batch ASW: {0:.3f}, Batch kBET: {1:.3f}".format(batch_sil, kbet),
        transform=ax.transAxes,
        horizontalalignment="center",
        verticalalignment="center",
    )
...
def train(args):
    ...
    model = GMVAE(
        encoder, decoder, dim_x, dim_w, args.K, 300, lamb=torch.Tensor([args.L])
    ).to(device)
    ...
    for i in range(1, args.num_epoche + 1):
        ...
        print(
            "epoch: {0:.0f}, loss: {1:.3f}, test loss: {2:.3f}".format(
                i, loss_ep, loss_ep_test
            )
        )
    ...
    ax = plt.axes()
    sns.scatterplot(
        ax=ax, x=np.arange(0, len(loss_arr)), y=loss_arr, label="train error"
    )
    sns.scatterplot(
        ax=ax, x=np.arange(0, len(loss_arr_test)), y=loss_arr_test, label="test error"
    )
    ...
    torch.save(
        model.state_dict(), "GMVAE_{}_{}_{}_{}.tc".format(dim_x, dim_w, args.K, args.L)
    )
    return plot(
        test_data,
        "umap_latent_{}_{}_{}_{}.png".format(dim_x, dim_w, args.K, args.L),
        dim_x,
