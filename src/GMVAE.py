...
class Encoder(nn.Module):
    ...
    def forward(self, inp: torch.Tensor) -> Tuple[Tuple[torch.Tensor]]:
        ...
        return (
            torch.chunk(self.infer_w(inp), 2, dim=-1),
...
class Decoder(nn.Module):
    ...
    def log_prob(self, x: torch.Tensor, y: torch.Tensor):
        ...
        normalization = (
            -torch.lgamma(tot_count + y)
            + torch.lgamma(1.0 + y)
            + torch.lgamma(tot_count)
        )
...
def KL_gaussian(
    mu_1: torch.Tensor,
    sigma_1_log: torch.Tensor,
    mu_2: torch.Tensor,
    sigma_2_log: torch.Tensor,
):
    """
    ...
    return torch.sum(
        -sigma_1_log
        + sigma_2_log
        + (torch.exp(2 * sigma_1_log) + (mu_1 - mu_2) ** 2)
        / (2 * torch.exp(sigma_2_log * 2))
        - 1 / 2,
        dim=-1,
    )
...
def KL_categorical(probs_1, probs_2):
    ...
    """
    return torch.sum(
        probs_1 * (torch.log(probs_1 + 1e-8) - torch.log(probs_2 + 1e-8)), dim=0
    )
...
class GMVAE(nn.Module):
    def __init__(
        ...
        self.GMM = nn.Sequential(
            nn.Linear(dim_w, GMM_hidden_dim),
            nn.GELU(),
            nn.LayerNorm(GMM_hidden_dim),
            nn.Dropout(0.05),
            nn.Linear(GMM_hidden_dim, 2 * K * dim_x),
        )
    ...
    def forward(self, y: torch.Tensor, M: int, lab_batch=None) -> torch.Tensor:
        ...
        x = (
            torch.randn([M, *x_mu.shape], device=self.device) * torch.exp(x_sigma_log)
            + x_mu
        )
        w = (
            torch.randn([M, *w_mu.shape], device=self.device) * torch.exp(w_sigma_log)
            + w_mu
        )
        means, log_std = torch.chunk(
            self.GMM(w).reshape(self.K, M, batch, self.dim_x * 2), 2, dim=3
        )
        std = torch.exp(log_std)
        log_probs = log_like_gaussian(means, std, x)
        z_posterior = F.softmax(log_probs, dim=0)
        KL_w_prior = KL_gaussian(
            w_mu, w_sigma_log, torch.zeros_like(w_mu), torch.zeros_like(w_sigma_log)
        )
        KL_z_prior = torch.maximum(
            KL_categorical(z_posterior, torch.ones_like(z_posterior) / self.K),
            self.lamb,
        )
        conditional_prior = torch.sum(
            z_posterior * KL_gaussian(x_mu, x_sigma_log, means, log_std), dim=0
        )
...
def score(predictions, width):
    score1 = silhouette_score(
        predictions.iloc[:, 0:width].values, labels=predictions["cell_type"].values
    )
    score_final = 0
    for batch in predictions["batch"].unique():
        batch_data = predictions.loc[predictions["batch"] == batch]
        sample_scores = 1 - np.abs(
            silhouette_samples(
                X=batch_data.iloc[:, 0:width].values,
                labels=batch_data["cell_type"].values,
            )
        )
